...
class AudioLDM2Dataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            split: If set ('train' or 'val'), only rows with manifest['split'] == split
                are loaded. If None, all rows pass. Manifests without a 'split'
                column (e.g. smoke datasets) are treated as if every row is 'train'.
        """
        ...
        with open(self.manifest_path, encoding='utf-8') as f:
            ...
            for row in reader:
                if row.get('status') != 'ok':
                    continue
                row_split = row.get('split', 'train')  # backward-compat
        ...
        if not self.items:
            raise ValueError(
                f"No valid items found in {self.manifest_path} "
                f"(split filter: {split!r})"
            )
    ...
    def __getitem__(self, idx: int):
        ...
        # soundfile returns (samples,) for mono or (samples, channels) for multichannel;
        # we transpose to torchaudio's (channels, samples) layout.
        audio_np, sr = sf.read(str(item['audio_path']), dtype="float32", always_2d=True)
