"""Train a small E-B weather calibration head on frozen PANNs/DSP features."""
...
from layers.layer_e.attempts.liting__mvp_2__calibrated_weather_head.code.weather_head import (  # noqa: E402
    FEATURE_NAMES,
    RAIN_CLASSES,
    WIND_CLASSES,
...
DEFAULT_MODEL_DIR = PROJECT_ROOT / "model" / "candidates" / "liting" / "mvp_2__calibrated_weather_head"
DEFAULT_DEBUG_DIR = PROJECT_ROOT / "debug" / "e_b_weather_mvp2"


def main() -> int:
    parser = argparse.ArgumentParser(description="Train E-B MVP-2 calibrated weather head.")
    ...
    checkpoint = {
        "attempt": "liting__mvp_2__calibrated_weather_head",
...
def train_component(
    ...
    y_train = torch.tensor([class_to_idx[rows[i][label_key]] for i in train_idx], dtype=torch.long)
...
def head_to_dict(model: nn.Linear) -> dict:
    return {
