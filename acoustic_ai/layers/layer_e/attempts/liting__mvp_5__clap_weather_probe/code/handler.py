"""Registry handler for Liting's E-B MVP-5 CLAP weather probe."""
...
DEFAULT_CHECKPOINT = PROJECT_ROOT / "model" / "candidates" / "liting" / "mvp_5__clap_weather_probe" / "weather_head.pt"
INTENSITY_VALUE = {
    "none": 0.0,
    "light": 0.25,
    "moderate": 0.6,
...
def load(checkpoint_dir: Path | None, params: dict, extra: dict | None = None) -> dict:
    ...
    if not checkpoint_path.exists():
        raise FileNotFoundError(
            "Missing checkpoint for liting__mvp_5__clap_weather_probe: "
            f"{checkpoint_path}. Run `python3 -m dvc pull "
            "model/candidates/liting/mvp_5__clap_weather_probe/weather_head.pt.dvc` "
...
def analyze(state: dict, audio_path: str | Path) -> dict:
    ...
    if not checkpoint:
        raise RuntimeError("MVP5 weather head checkpoint is not loaded; refusing to fall back to another model.")

    features, evidence = extract_feature_vector(audio_path, clap=state.get("clap"))
    prediction = predict_with_checkpoint(features, checkpoint)
    weather = {
        "component": "E-B",
        "method": "clap_dsp_frozen_features__weather_probe",
        "primary_model": "LAION-CLAP frozen audio embedding + E-B weather probe",
        **prediction,
        "clap_available": evidence["clap_available"],
        "clap_status": evidence["clap_status"],
        "supporting_detector": "DSP features included alongside CLAP embedding",
        "limitations": [
            "MVP-5 trains small heads over frozen CLAP/DSP features on audited Site257 weather clips.",
            "The CLAP backbone is not fine-tuned.",
