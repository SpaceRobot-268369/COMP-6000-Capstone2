"""Train E-B MVP-5 CLAP weather probe on frozen CLAP/DSP features."""
...
from layers.layer_e.attempts.liting__mvp_5__clap_weather_probe.code.clap_backbone import CLAPBackbone  # noqa: E402
from layers.layer_e.attempts.liting__mvp_5__clap_weather_probe.code.weather_head import (  # noqa: E402
    FEATURE_NAMES,
    RAIN_CLASSES,
    WIND_CLASSES,
...
DEFAULT_MODEL_DIR = PROJECT_ROOT / "model" / "candidates" / "liting" / "mvp_5__clap_weather_probe"
DEFAULT_DEBUG_DIR = PROJECT_ROOT / "debug" / "e_b_weather_mvp5"
DEFAULT_AUDIO_SUFFIXES = (".wav", ".webm", ".flac", ".mp3")


def main() -> int:
    parser = argparse.ArgumentParser(description="Train E-B MVP-5 CLAP weather probe.")
    ...
    vectors = []
    for asset in assets:
        vector, evidence = extract_feature_vector(asset.audio_path, clap=clap)
        ...
        rows.append(
            {
                ...
                "split_role": "audited",
                "clap_available": evidence["clap_available"],
                "clap_status": evidence["clap_status"],
            }
        )
        print(
            f"[feature] {asset.asset_id}: rain={rows[-1]['rain_label']} "
            f"wind={rows[-1]['wind_label']} policy={rows[-1]['policy_class']} "
            f"clap={rows[-1]['clap_available']}"
        )
    ...
    checkpoint = {
        "attempt": "liting__mvp_5__clap_weather_probe",
...
def build_pseudo_labelled_rows(
    ...
    if roots:
        print("NOTE: MVP5 CLAP probe is audited-only; pseudo-labelled expansion is ignored for this attempt.")
    return [], []
...
def evaluate(X: np.ndarray, rows: list[dict], val_idx: list[int], checkpoint: dict) -> tuple[list[dict], dict]:
    ...
    return eval_rows, {
        ...
        "demo_summary": {
            "model": "Frozen LAION-CLAP audio embedding + DSP features + rain/wind probe heads",
            "training_mode": "Server B training over audited Site257 weather labels only",
            "output_contract": {
...
def classify_gate(
    ...
    return {
        ...
        "interpretation": (
            "MVP5 is usable for demo and further calibration."
            if passed
            else "MVP5 should be treated as an iteration checkpoint, not a final detector."
        ),
    }
