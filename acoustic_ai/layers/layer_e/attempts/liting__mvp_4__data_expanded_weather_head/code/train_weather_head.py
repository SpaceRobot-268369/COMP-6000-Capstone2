"""Train E-B MVP-4 data-expanded weather heads on frozen PANNs/DSP features."""
...
from layers.layer_e.attempts.liting__mvp_4__data_expanded_weather_head.code.weather_head import (  # noqa: E402
    FEATURE_NAMES,
    RAIN_CLASSES,
    WIND_CLASSES,
...
DEFAULT_MODEL_DIR = PROJECT_ROOT / "model" / "candidates" / "liting" / "mvp_4__data_expanded_weather_head"
DEFAULT_DEBUG_DIR = PROJECT_ROOT / "debug" / "e_b_weather_mvp4"
DEFAULT_AUDIO_SUFFIXES = (".wav", ".webm", ".flac", ".mp3")


def main() -> int:
    parser = argparse.ArgumentParser(description="Train E-B MVP-4 data-expanded weather head.")
    ...
    parser.add_argument("--extra-audio-root", action="append", type=Path, default=[])
    parser.add_argument("--extra-scan-limit", type=int, default=300)
    parser.add_argument("--pseudo-per-bucket", type=int, default=40)
    ...
    print(f"audited_assets={len(assets)} index={args.asset_index}")
        ...
        rows.append(
            {
                ...
                "audio_path": display_path(asset.audio_path),
                ...
                "label_source": "audited_layer_b_site_asset",
                "split_role": "audited",
    ...
    extra_rows, extra_vectors = build_pseudo_labelled_rows(
        args.extra_audio_root,
        existing_paths={Path(row["audio_path"]).name for row in rows},
        scan_limit=args.extra_scan_limit,
        per_bucket=args.pseudo_per_bucket,
        seed=args.seed,
    )
    rows.extend(extra_rows)
    vectors.extend(extra_vectors)
    print(f"pseudo_assets={len(extra_rows)} total_cases={len(rows)}")
    feature_seconds = time.perf_counter() - feature_start
    component_rng_state = torch.random.get_rng_state()

    X = np.vstack(vectors).astype(np.float32)
    audited_train_idx, val_idx = make_split(rows[:audited_count], args.val_frac, args.seed)
    pseudo_idx = list(range(audited_count, len(rows)))
    train_idx = sorted(audited_train_idx + pseudo_idx)
    ...
    checkpoint = {
        "attempt": "liting__mvp_4__data_expanded_weather_head",
    ...
    metrics.update(
        {
            ...
            "case_count": len(rows),
            "audited_case_count": audited_count,
            "pseudo_case_count": len(extra_rows),
            "train_count": len(train_idx),
            "val_count": len(val_idx),
            "extra_audio_roots": [str(path) for path in args.extra_audio_root],
            "extra_scan_limit": args.extra_scan_limit,
            "pseudo_per_bucket": args.pseudo_per_bucket,
            "timings": timings,
...
def build_pseudo_labelled_rows(
    roots: list[Path],
    ...
    candidates = discover_extra_audio(roots, existing_paths, scan_limit, seed)
    if not candidates:
        return [], []
    ...
    for path in candidates:
        try:
            vector, evidence = extract_feature_vector(path)
        except Exception as exc:  # noqa: BLE001 - skip unreadable external audio during data expansion.
            print(f"[pseudo-skip] {path}: {exc}")
            continue
        scores = evidence["panns_scores"]
        scored.append(
            {
                "path": path,
                "vector": vector,
                "rain_score": float(scores.get("rain", 0.0)),
                "wind_score": float(scores.get("wind", 0.0)),
                "thunder_score": float(scores.get("thunder", 0.0)),
                "panns_available": evidence["panns_available"],
                "panns_status": evidence["panns_status"],
            }
        )
    ...
    rain_ranked = sorted(
        scored,
        key=lambda item: (item["rain_score"] - item["wind_score"], item["rain_score"]),
        reverse=True,
    )
    for item in rain_ranked:
        if len([entry for entry in selected if entry[0] == "pseudo_rain"]) >= per_bucket:
            break
        if item["path"] in used or item["rain_score"] < 0.08:
            continue
        selected.append(("pseudo_rain", "moderate", "none", item))
        used.add(item["path"])

    wind_ranked = sorted(
        scored,
        key=lambda item: (item["wind_score"] - item["rain_score"], item["wind_score"]),
        reverse=True,
    )
    ...
    for item in quiet_ranked:
        if len([entry for entry in selected if entry[0] == "pseudo_no_weather"]) >= per_bucket:
            break
        if item["path"] in used:
            continue
        selected.append(("pseudo_no_weather", "none", "none", item))
        used.add(item["path"])

    rows = []
    vectors = []
    for idx, (bucket, rain_label, wind_label, item) in enumerate(selected):
        path = item["path"]
        rows.append(
            {
                "asset_id": f"mvp4_{bucket}_{idx:04d}",
                "audio_path": display_path(path),
                "rain_label": rain_label,
                "wind_label": wind_label,
                "policy_class": classify_labels(rain_label, wind_label),
                "label_source": "panns_pseudo_label_site257",
                "split_role": "pseudo_train_only",
                "panns_available": item["panns_available"],
                "panns_status": item["panns_status"],
                "pseudo_bucket": bucket,
                "pseudo_rain_score": round(item["rain_score"], 6),
                "pseudo_wind_score": round(item["wind_score"], 6),
                "pseudo_thunder_score": round(item["thunder_score"], 6),
            }
        )
...
def discover_extra_audio(roots: list[Path], existing_paths: set[str], limit: int, seed: int) -> list[Path]:
    discovered: list[Path] = []
    for root in roots:
        if not root.exists():
            continue
        for path in root.rglob("*"):
            if path.suffix.lower() not in DEFAULT_AUDIO_SUFFIXES:
                continue
...
def classify_policy(asset) -> str:
    return classify_labels(asset.labels.get("rain", "none"), asset.labels.get("wind", "none"))


def classify_labels(rain_label: str, wind_label: str) -> str:
    rain = rain_label != "none"
    wind = wind_label != "none"
    if rain and wind:
...
def evaluate(X: np.ndarray, rows: list[dict], val_idx: list[int], checkpoint: dict) -> tuple[list[dict], dict]:
    ...
    return eval_rows, {
        ...
        "demo_summary": {
            "model": "Frozen PANNs CNN14 + DSP features + data-expanded rain/wind heads",
            "training_mode": "Server B training over audited Site257 weather labels plus pseudo-labelled Site257 train-only clips",
            "output_contract": {
...
def classify_gate(
    ...
    return {
        ...
        "interpretation": (
            "MVP4 is usable for demo and further calibration."
            if passed
            else "MVP4 should be treated as an iteration checkpoint, not a final detector."
        ),
    }
