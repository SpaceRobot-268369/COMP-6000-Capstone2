...
class EventScheduler:
    ...
    def _schedule_ecological(self, snippets: list[RetrievedSnippet]) -> list[ScheduledEvent]:
        """Schedule snippets in loose species-specific bouts.

        This keeps Layer C sparse while avoiding metronomic spacing. The values
        are deliberately conservative for the smoke demo: retrieval snippets are
        real audited calls, so ecological plausibility here mostly means natural
        gaps, occasional short bouts, and no dense wall of events.
        """
    ...
    def _profile_for(event_type: str) -> dict[str, tuple[float, float] | tuple[int, int] | float]:
        ...
        if "boobook" in event_key:
            return {
                "start_window_s": (5.0, 10.0),
                "bout_size": (1, 3),
                "within_bout_gap_s": (3.5, 7.0),
                "between_bout_gap_s": (10.0, 18.0),
                "gain_db": (-6.0, -2.5),
                "fade_s": (0.10, 0.20),
                "end_margin_s": 2.0,
            }
        ...
        payload = {
            ...
            "scheduling_params": {
                **request,
                ...
                "ecological_mode": self.ecological_mode,
            },
        }
