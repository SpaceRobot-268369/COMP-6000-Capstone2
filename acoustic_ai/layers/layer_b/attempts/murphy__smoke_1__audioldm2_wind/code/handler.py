"""Layer B attempt handler — AudioLDM2 LoRA, wind-only smoke test.
...
def generate(state: _State, seed: Optional[int] = None, **_ignored) -> dict:
    """Generate one weather wind stem.
    ...
    smoke contract for the wind-only LoRA.
    """
    p = state.params
    prompt              = p["prompt"]
    ...
    metadata = {
        "generator":             "audioldm2_lora_layer_b_wind",
        "prompt_locked":         True,
        "prompt":                prompt,
