...
direct_water_L = active_power_kW * runtime_hours * WUE
```
...
| `PUE` | `1.15` | AWS 2024 global PUE. The AWS public metrics table lists Sydney 2024 PUE as `--`, so this uses the global figure rather than inventing a Sydney-specific value. |
| `WUE` | `0.12 L/kWh IT` | AWS 2024 Asia-Pacific (Sydney) WUE from the public AWS region PUE/WUE table. |
...
```text
0.25 * 1.15 * 0.4 = 0.115 kgCO2e/hour
0.25 * 0.12 = 0.030 L direct AWS water/hour
```

Public source for the PUE/WUE assumptions:
...
| 16-cell Layer A LoRA bank | 16 x same recipe | 2,496 s | ~0.080 kgCO2e |
| Conservative training budget | Server B default budget | 60 min | ~0.115 kgCO2e |
| Conservative long budget | Server B default upper budget | 120 min | ~0.230 kgCO2e |

Direct AWS water estimate:

| Scope | Runtime basis | Runtime | Estimate |
|---|---:|---:|---:|
| Single Layer A LoRA cell | measured `mvp_1_1` diagnostic run on `shinypokemon` Tesla T4 | 156 s | ~0.001 L |
| 16-cell Layer A LoRA bank | 16 x same recipe | 2,496 s | ~0.021 L |
| Conservative training budget | Server B default budget | 60 min | ~0.030 L |
| Conservative long budget | Server B default upper budget | 120 min | ~0.060 L |
...
| Idle tail after request | Server B idle timeout | 15 min | ~0.029 kgCO2e |
