# layer-c-sa3-horsfields-bronze-cuckoo-pass35-captionv2-t2a1000

## Summary

- Owner: burger
- Layer / role: Layer C event generation, Horsfield's Bronze-cuckoo
- Status: candidate
...
Stable Audio 3 LoRA candidate for live Layer C generation of isolated
Horsfield's Bronze-cuckoo calls. The registered runtime uses this checkpoint
for request-time generation before species-specific post-processing.

## Dataset / inputs

- Dataset: Horsfield's Bronze-cuckoo SA3 LoRA training set
- Source clips / manifests: not recorded in this artifact
- Filtering or preprocessing: pass-35 caption v2 training setup
...
- Important settings: checkpoint step 1000; see `params.yaml`

## Artifacts

- Checkpoint binaries:
  - `lora_checkpoints/epoch=28-step=1000.ckpt`
- DVC pointer files:
  - `lora_checkpoints/epoch=28-step=1000.ckpt.dvc`
- Params: `params.yaml`
- Metrics: not provided
- Sample outputs:
  - Expected: `acoustic_ai/layers/layer_c/attempts/burger__mvp_3__sa3_generative_live/expected/horsfields_bronze_cuckoo/`
