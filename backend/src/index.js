...
const LAYER_B_GENERATE_ATTEMPTS = new Set(
  (process.env.LAYER_B_GENERATE_ATTEMPTS ||
    "murphy__smoke_1__audioldm2_wind,murphy__smoke_1__audioldm2_rain,murphy__mvp_1__wind_intensity_bank")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean),
);
const LAYER_B_GENERATE_UNAVAILABLE_ATTEMPTS = new Set(
  (process.env.LAYER_B_GENERATE_UNAVAILABLE_ATTEMPTS || "murphy__smoke_1__audioldm2_wind")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean),
);
  ...
  const generateAttempts = generateLayerB.attempts
    ...
    .map((attempt) => {
      ...
      return {
        ...
        available: false,
        unavailable_reason:
          "Superseded smoke attempt. Use murphy__mvp_1__wind_intensity_bank for Layer B wind generation testing.",
      };
    });
